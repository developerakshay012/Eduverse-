...
export const editCourse = async (req, res) => {
  try {
    ...
    // SAFE UPDATE LOGIC
    // updates.hasOwnProperty ki jagah Object.keys use kiya hai taki crash na ho
    // const { courseId } = req.body
// const updates = { ...req.body } 
...
export const markLectureAsComplete = async (req, res) => {
    // FIX 1: subSectionId ko body se nikala (CamelCase) aur userId ko req.user se nikala
    const { courseId, subSectionId } = req.body
    const userId = req.user.id // Middleware se userId li (Safe tarika)

    // Validation check se userId hata diya kyunki ye middleware handle kar raha hai
    if (!courseId || !subSectionId) {
      return res.status(400).json({
        success: false,
        message: "Missing required fields: courseId or subSectionId",
      })
    }
    ...
    try {
      ...
      // Check agar video pehle se hi completed hai
      ...
      // Agar complete nahi hai, toh array mein push karein
