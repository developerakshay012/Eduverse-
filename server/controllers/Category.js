...
export const categoryPageDetails = async (req, res) => {
    try {
        ...
        // Handle the case when there are no courses: return 200 with empty arrays instead of 404
        if (selectedCategory.courses.length === 0) {
            return res.status(200).json({
                success: true,
                selectedCategory: selectedCategory, // Send category data even if courses are empty
                selectedCourses: [],
                differentCourses: [],
                mostSellingCourses: [],
